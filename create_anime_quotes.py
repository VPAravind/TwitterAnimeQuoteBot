...
def get_quotes():
    ...
    lists = quote[random.randint(0, len(quote)-1)]
    return lists
...
Character: {character}
Anime: {anime}
Source: https://animechanapi.xyz/api/quotes/random
#AnimeQuotes'''
    return tweet

def create_anime_tweet():
    ...
    while len(tweet) == 0 or len(tweet) > 200:
        rand = random.randint(0,1)
        if rand == 0:
            try:
                response = requests.get('https://animechanapi.xyz/api/quotes/random')
                quote = response.json()['data'][0]['quote']
                character = response.json()['data'][0]['character']
                anime = response.json()['data'][0]['anime']
                tweet = f'''Quote:'{quote}'
Character: {character}
Anime: {anime}
Source: https://animechanapi.xyz/api/quotes/random
